...
export default function DealsApp() {
      ...
      try {
        ...
        if (data && data.length > 0) {
          const formatted = data.map((d: any) => ({
            ...
            storeLocation:
              typeof d.storeLocation === "string"
                ? JSON.parse(d.storeLocation)
                : d.storeLocation || {
                    lat: 34.0522,
                    lng: -118.2437,
                    city: "US",
  ...
  const [mobileActiveTab, setMobileActiveTab] = useState<"listings" | "map" | "promoter">(
    "listings",
  );
  ...
  const [sortBy, setSortBy] = useState<"discount" | "price-low" | "price-high" | "popularity">(
    "discount",
  );
  ...
  const [userStats, setUserStats] = useState<UserStats>({
    ...
    totalEarnings: 215.4,
    pendingPayout: 140.4,
    payoutHistory: [{ id: "pay-7128", date: "Jun 15, 2026", amount: 75.0, status: "Completed" }],
  });
  ...
  const activeDeal = useMemo(() => {
    return (
      DEALS_DATA.find((d) => d.id === activeDealId) ||
      DEALS_DATA[0] || {
        id: "",
  ...
  const filteredDeals = useMemo(() => {
    return DEALS_DATA.filter((deal) => {
      ...
      // Region matching
      const matchesRegion =
        selectedRegion === "All Regions" || deal.storeLocation.city === selectedRegion;
  ...
  return (
    <div className="min-h-screen bg-neutral-100 text-neutral-900 font-sans antialiased flex flex-col">
      ...
      <header
        id="main-header"
        className="bg-neutral-900 text-white shadow-md border-b border-neutral-800 sticky top-0 z-40"
      >
        <div className="max-w-7xl mx-auto px-4 py-3.5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-bold uppercase tracking-widest bg-teal-400/20 text-teal-300 px-2 py-0.5 rounded border border-teal-500/30">
                  USA LIQUIDATION
                </span>
              </div>
          ...
          <div className="flex items-center gap-4 text-xs">
            ...
            <div className="flex items-center gap-2.5 bg-teal-950/40 border border-teal-900 px-3 py-1.5 rounded-xl text-teal-300">
              ...
              <div>
                <span className="text-[9px] uppercase font-bold text-teal-500 block leading-none">
                  Your Promoter Earnings
                </span>
      ...
      <main className="max-w-7xl mx-auto w-full px-4 py-6 flex-grow flex flex-col gap-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <h3 className="text-sm font-bold text-neutral-900 mt-0.5">
                Find & Promote Open-Box Liquidation Packages
              </h3>
              <p className="text-xs text-neutral-500 leading-relaxed mt-0.5">
                Toggle through deals in major US regions, compare prices with local comps, and
                simulate the exact pickup drive-time. Want to earn? Share the trackable referral
                links to direct buyers—and watch your commission dashboard grow instantly!
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4">
            ...
            {/* LISTINGS PANEL */}
            <div
              className={`space-y-3 lg:block ${mobileActiveTab === "listings" ? "block" : "hidden"}`}
            >
          ...
          <div className="lg:col-span-7 space-y-6 flex flex-col justify-start">
            ...
            <div
              className={`bg-neutral-900 text-white rounded-2xl border border-neutral-800 p-5 shadow-lg flex flex-col md:flex-row items-center justify-between gap-4 lg:block ${mobileActiveTab === "map" ? "block" : "hidden"}`}
            >
              <div className="space-y-1.5 text-center md:text-left">
                ...
                <p className="text-xs text-neutral-400 leading-normal max-w-lg">
                  Route yourself to the retail check-out page. Completing this purchase triggers the
                  referral commission logic directly under your promoter stats.
                </p>
              </div>
            ...
            <div className={`lg:block ${mobileActiveTab === "promoter" ? "block" : "hidden"}`}>
              {user ? (
                <EarningsHub deals={DEALS_DATA} userStats={userStats} setUserStats={setUserStats} />
              ) : null}
            </div>
          </div>
        </div>
      ...
      <footer className="border-t border-neutral-200 bg-white py-10 mt-16 text-center text-xs text-neutral-500 space-y-4">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="font-extrabold text-neutral-800 tracking-tight">
              OpenDeal Route Scanner
            </span>
            <span>•</span>
        ...
        <p className="text-[10px] text-neutral-400 max-w-xl mx-auto leading-relaxed px-4">
          OpenDeal Scanner is an interactive demonstration showcasing automated geo-location package
          deal scouting, custom drive time computations, competitive localized pricing grids
          (comps), and direct affiliate commission tracking interfaces.
        </p>
      </footer>
        ...
        <div className="fixed inset-0 bg-neutral-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl border border-neutral-200 max-w-md w-full shadow-2xl overflow-hidden p-6 space-y-5 animate-[scaleIn_0.2s_ease-out_1]">
            ...
            <div className="space-y-3 text-center">
              ...
              <div>
                <h3 className="font-bold text-sm text-neutral-950 line-clamp-2">
                  {claimedDeal.title}
                </h3>
                <p className="text-xs text-neutral-500 mt-1">
                  Visit Merchant:{" "}
                  <strong className="text-neutral-800">{claimedDeal.storeName}</strong>
                </p>
              </div>
            </div>
            ...
            <div className="bg-teal-50 border border-teal-150 rounded-2xl p-4 text-xs text-teal-900 space-y-2">
              ...
              <p className="leading-relaxed text-[11px] text-teal-800">
                The URL has been injected with the promoter parameter{" "}
                <code className="font-mono font-bold bg-teal-100 px-1 rounded">
                  promoter={referralPromoterInput}
                </code>
                . Complete this checkout simulation to immediately update the affiliate dashboard
                with earned funds!
              </p>
              <div className="flex justify-between items-center py-1 border-t border-teal-200/50 pt-2 font-mono text-[10px] text-teal-700">
                ...
                <span className="font-extrabold text-teal-900">
                  ${claimedDeal.referralEarningValue.toFixed(2)}
                </span>
              </div>
            </div>
...
function AuthChip() {
  ...
  if (loading) {
    return (
      <div
        className="hidden sm:block h-8 w-20 rounded-xl bg-neutral-800 animate-pulse"
        aria-hidden
      />
    );
  }
